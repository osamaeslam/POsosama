...
export const ProductsScreen: React.FC = () => {
  ...
  const handleAddCategory = () => {
    ...
    if (!name) return;
    addCategory({ name, color: newCategoryColor, sortOrder: categories.length });
    setNewCategoryName('');
  };
  ...
  const handleDeleteCategory = (id: string) => {
    try {
      ...
      window.alert(error instanceof Error ? error.message : 'تعذر حذف الفئة');
    }
  };
    ...
    <div className="p-6 space-y-6 overflow-y-auto max-w-7xl mx-auto w-full">
      ...
      <section className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-bold text-slate-900">إدارة الفئات</h3>
            <p className="text-[11px] text-slate-500">أضف فئاتك بنفسك، وعدّل الاسم أو اللون من هنا.</p>
          </div>
          <span className="text-[11px] font-bold text-slate-500">{categories.length} فئة</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {categories.map((category) => (
            <div key={category.id} className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5">
              {editingCategoryId === category.id ? (
                <input
                  autoFocus
                  defaultValue={category.name}
                  onBlur={(event) => { updateCategory(category.id, { name: event.target.value.trim() || category.name }); setEditingCategoryId(null); }}
                  onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur(); }}
                  className="w-28 bg-white border border-blue-300 rounded px-1.5 py-1 text-xs font-semibold outline-none"
                />
              ) : (
                <button type="button" onClick={() => setEditingCategoryId(category.id)} className="text-xs font-semibold text-slate-700 cursor-pointer hover:text-blue-700">{category.name}</button>
              )}
              <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: category.color }} />
              <button type="button" onClick={() => handleDeleteCategory(category.id)} aria-label={`حذف فئة ${category.name}`} className="text-slate-400 hover:text-rose-600 cursor-pointer"><XCircle className="h-3.5 w-3.5" /></button>
            </div>
          ))}
        </div>
        <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
          <input value={newCategoryName} onChange={(event) => setNewCategoryName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') handleAddCategory(); }} placeholder="اسم الفئة الجديدة" className="min-w-44 flex-1 px-3 py-2 border border-slate-200 rounded-lg text-xs font-semibold outline-none focus:ring-2 focus:ring-blue-500" />
          <input type="color" value={newCategoryColor} onChange={(event) => setNewCategoryColor(event.target.value)} aria-label="لون الفئة" className="h-9 w-12 cursor-pointer rounded border border-slate-200 bg-white p-1" />
          <button type="button" onClick={handleAddCategory} className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold cursor-pointer"><Plus className="inline h-3.5 w-3.5" /> إضافة فئة</button>
        </div>
      </section>
